...
class LocalStorageService {
  ...
  Future<void> trackListened(String trackId, String title) async {
    ...
    const String keyHistory = 'listening_history_list';
    final historyStr = prefs.getString(keyHistory) ?? '[]';
    ...
    await prefs.setString(keyHistory, json.encode(history));
  }

  Future<List<Map<String, dynamic>>> getHistory() async {
    final prefs = await SharedPreferences.getInstance();
    final historyStr = prefs.getString('listening_history_list') ?? '[]';
    return (json.decode(historyStr) as List).cast<Map<String, dynamic>>();
  }
